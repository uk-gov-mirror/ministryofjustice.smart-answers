...
module SmartAnswer
  class Flow
    ...
    def use_quick_escape_button(use_quick_escape_button) # rubocop:disable Style/TrivialAccessors
      @use_quick_escape_button = use_quick_escape_button
    end

    def use_quick_escape_button?
      raise NonSessionBasedFlow, "This flow is not session-based" unless response_store == :session

      ActiveModel::Type::Boolean.new.cast(@use_quick_escape_button)
    end

    def show_quick_escape_banner?
      response_store == :session && use_quick_escape_button?
    end
