...
multiple_choice :which_benefit? do
  ...
  on_condition(->(_) { going_or_already_abroad == 'going_abroad' }) do
  ...
  on_condition(->(_) { going_or_already_abroad == 'already_abroad' }) do
...
multiple_choice :channel_islands? do
  ...
  on_condition(responded_with('abroad')) do
    next_node_if(:which_country_jsa?) { benefit == 'jsa' }
    next_node_if(:which_country_maternity_benefits?) { benefit == 'maternity_benefits' }
    next_node_if(:which_country_child_benefit?) { benefit == 'child_benefit' }
    next_node_if(:which_country_iidb?) { benefit == 'iidb' }
    next_node_if(:which_country_bereavement_benefits?) { benefit == 'bereavement_benefits' }
  end
  on_condition(responded_with('guernsey_jersey')) do
    on_condition(->(_) { going_or_already_abroad == 'going_abroad' } ) do
      next_node_if(:jsa_social_security_going_abroad_outcome) { benefit == 'jsa' }
      next_node_if(:iidb_going_abroad_ss_outcome) { benefit == 'iidb' }
      next_node_if(:bb_going_abroad_ss_outcome) { benefit == 'bereavement_benefits' }
    end

    next_node_if(:jsa_social_security_already_abroad_outcome) { benefit == 'jsa' }
    next_node_if(:employer_paying_ni?) { benefit == 'maternity_benefits' }
    next_node_if(:child_benefit_ss_outcome) { benefit == 'child_benefit' }
    next_node_if(:iidb_already_abroad_ss_outcome) { benefit == 'iidb' }
    next_node_if(:bb_already_abroad_ss_outcome) { benefit == 'bereavement_benefits' }
  end
end
...
country_select :which_country_jsa?, :exclude_countries => exclude_countries do
  ...
  on_condition(->(_) { going_or_already_abroad == 'already_abroad' } ) do
    next_node_if(:jsa_eea_already_abroad_outcome) { |response| eea_countries.include?(response) } # A3 or A4
    next_node_if(:jsa_social_security_already_abroad_outcome) { |response| social_security_countries_jsa.include?(response) } # A5 or A6
  end

  on_condition(->(_) { going_or_already_abroad == 'going_abroad' } ) do
    next_node_if(:jsa_eea_going_abroad_outcome) { |response| eea_countries.include?(response) } # A3 or A4
    next_node_if(:jsa_social_security_going_abroad_outcome) { |response| social_security_countries_jsa.include?(response) } # A5 or A6
  end
...
country_select :which_country_wfp?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:wfp_eea_eligible_outcome) { |response| eea_countries.include?(response) } # A10
...
country_select :which_country_maternity_benefits?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:working_for_a_uk_employer?) { |response| eea_countries.include?(response) }
...
multiple_choice :employer_paying_ni? do
  ...
  on_condition(->(_) { social_security_countries_mat.include?(country) }) do
    on_condition(->(_) { going_or_already_abroad == 'already_abroad' } ) do
...
country_select :which_country_child_benefit?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:do_either_of_the_following_apply?) { |response| eea_countries.include?(response) } # Q10
  on_condition(->(response) { former_yugoslavia.include?(response) }) do
  ...
  next_node_if(:child_benefit_ss_outcome) { |response| %w(barbados canada israel mauritius new-zealand).include?(response) } # A19
  next_node_if(:child_benefit_jtu_outcome) { |response| %w(jamaica turkey usa).include?(response) } # A20
...
country_select :which_country_ssp?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:working_for_uk_employer_ssp?) { |response| eea_countries.include?(response) } # Q12
...
country_select :which_country_tax_credits?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:tax_credits_currently_claiming?) { |response| eea_countries.include?(response) } # Q18
...
country_select :which_country_esa?, :exclude_countries => exclude_countries do
  ...
  on_condition(->(_) { going_or_already_abroad == 'going_abroad'}) do
    next_node_if(:esa_going_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A37
    next_node(:esa_going_abroad_other_outcome) # A39
  end
  on_condition(->(_) { going_or_already_abroad == 'already_abroad'}) do
    next_node_if(:esa_already_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A38
...
country_select :which_country_iidb?, :exclude_countries => exclude_countries do
  ...
  on_condition(->(_) { going_or_already_abroad == 'going_abroad'}) do
    next_node_if(:iidb_going_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A42
    next_node_if(:iidb_going_abroad_ss_outcome) { |response| social_security_countries_iidb.include?(response) } # A44
    next_node(:iidb_going_abroad_other_outcome) # A46
  end
  on_condition(->(_) { going_or_already_abroad == 'already_abroad'}) do
    next_node_if(:iidb_already_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A43
    next_node_if(:iidb_already_abroad_ss_outcome) { |response| social_security_countries_iidb.include?(response) } # A45
...
country_select :which_country_disability_benefits?, :exclude_countries => exclude_countries do
  ...
  next_node_if(:db_claiming_benefits?) { |response| eea_countries.include?(response) }
...
country_select :which_country_bereavement_benefits?, :exclude_countries => exclude_countries do
  ...
  on_condition(->(_) { going_or_already_abroad == 'going_abroad'}) do
    next_node_if(:bb_going_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A54
    next_node_if(:bb_going_abroad_ss_outcome) { |response| social_security_countries_bereavement_benefits.include?(response) } # A56
    next_node(:bb_going_abroad_other_outcome) # A58
  end
  on_condition(->(_) { going_or_already_abroad == 'already_abroad'}) do
    next_node_if(:bb_already_abroad_eea_outcome) { |response| eea_countries.include?(response) } # A55
    next_node_if(:bb_already_abroad_ss_outcome) { |response| social_security_countries_bereavement_benefits.include?(response) } # A57
