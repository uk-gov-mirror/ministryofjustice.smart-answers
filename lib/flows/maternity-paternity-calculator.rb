...
date_question :payday_eight_weeks? do
  ...
  next_node_if(:pay_frequency?) { payday_exit == 'maternity' }
  next_node_if(:employees_average_weekly_earnings_paternity?) { payday_exit == 'paternity' }
...
multiple_choice :how_do_you_want_the_smp_calculated? do
  ...
  on_condition(responded_with("usual_paydates")) do
    next_node_if(:when_in_the_month_is_the_employee_paid?) { pay_pattern == "monthly" }
