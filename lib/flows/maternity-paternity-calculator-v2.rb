...
multiple_choice :employee_on_payroll_paternity? do
  option :yes => :employee_still_employed_on_birth_date? #previous:last_normal_payday? # NOTE: this goes to a shared question => QM5.2
...
multiple_choice :employee_still_employed_on_birth_date? do
  ...
  option :no
  calculate :employee_still_employed do
    if responses.last == 'no'
      pay_info = PhraseList.new (:paternity_not_entitled_to_pay_intro)
